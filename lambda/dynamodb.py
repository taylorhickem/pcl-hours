...
class DynamoDBTable(object):
    ...
    def query_by_decimal_value_eq(self, column_name, decimal_value, is_key=True, batch_max=None):
        if is_key:
            return self.query_by_key_decimal_value_eq(column_name, decimal_value, batch_max=batch_max)
        else:
            return self.query_by_attr_decimal_value_eq(column_name, decimal_value, batch_max=batch_max)
